...
class DataHandler:
    ...
    DATA_PATH = '/media/learner/6663-3462/WESAD/'
    #DATA_PATH = r'C:\WESAD'
    ...
    FEATURE_ATTRS =     ['max',  'min', 'mean', 'range', 'std']
    ACC_FEATURE_ATTRS = ['maxx', 'maxy', 'maxz', 'mean', 'std']

    # Keys for measurements collected by the RespiBAN on the chest
    # minus the ones we don't want
    # SENSOR_INPUTS = ['ACC','ECG','EDA','EMG','Resp','Temp']
    SENSOR_INPUTS = ['ACC', 'EDA','Temp']
    
    BASE_FEATURES = {'a_mean': [], 'a_std': [], 'a_maxx': [], 'a_maxy': [], 'a_maxz': [],\
                'e_max': [],  'e_min': [], 'e_mean': [], 'e_range': [], 'e_std': [], \
                't_max': [],  't_min': [], 't_mean': [], 't_range': [], 't_std': [] }
    STRESS_RESP_FEATURES = {'a_mean': [], 'a_std': [], 'a_maxx': [], 'a_maxy': [], 'a_maxz': [],\
                'e_max': [],  'e_min': [], 'e_mean': [], 'e_range': [], 'e_std': [], \
                't_max': [],  't_min': [], 't_mean': [], 't_range': [], 't_std': [] }
    ...
    def get_subject_path(self, subject):
        ...
        path = os.path.join(DataHandler.DATA_PATH, 'S'+ str(subject), 'S' + str(subject) + DataHandler.FILE_EXT)
    ...
    def load_all(self, subjects=PARTICIPANTS):
        """ 
        Parameters:
    ...
    def extract_and_reform(self, data, subject):
        ...
        baseline_indices = np.nonzero(data['label']==DataHandler.NORMAL)[0]   
        stress_indices = np.nonzero(data['label']==DataHandler.ANXIETY)[0]
        base = dict()
        stress = dict()
        
        for value in DataHandler.SENSOR_INPUTS: 
            base[value] = data['signal']['chest'][value][baseline_indices]
            stress[value] = data['signal']['chest'][value][stress_indices]
        
        DataHandler.BASELINE_DATA.append(base)
        DataHandler.STRESS_DATA.append(stress)
...
#              "of ", str(window_size) + "." )        
    ...
    def compute_features(self, subjects=PARTICIPANTS, data=BASELINE_DATA, window_size=42000, window_shift=175):
        """ 
        ...
        """
        keys = list(DataHandler.BASE_FEATURES.keys())
        ...
        for subject in subjects:
            ...
            for feature in DataHandler.ACC_FEATURE_ATTRS:
                #print('computed ', len(acc[feature]), 'windows for acc ', feature)
                DataHandler.BASE_FEATURES[keys[key_index]].extend(acc[feature])
                key_index = key_index + 1
            
            eda = self.get_stats(data[index]['EDA'], window_size, window_shift)
            for feature in DataHandler.FEATURE_ATTRS:
                #print('computed ', len(eda[feature]), 'windows for eda ', feature)
                DataHandler.BASE_FEATURES[keys[key_index]].extend(eda[feature])
                key_index = key_index + 1

            temp = self.get_stats(data[index]['Temp'], window_size, window_shift)
            for feature in DataHandler.FEATURE_ATTRS:
                #print('computed ', len(temp[feature]), 'windows for temp ', feature)
                DataHandler.BASE_FEATURES[keys[key_index]].extend(temp[feature])
                key_index = key_index + 1
            
        return DataHandler.BASE_FEATURES

    def compute_features_stress(self, subjects=PARTICIPANTS, data=STRESS_DATA, window_size=42000, window_shift=175):
        keys = list(DataHandler.STRESS_RESP_FEATURES.keys())
        ...
        for subject in subjects:
            ...
            for feature in DataHandler.ACC_FEATURE_ATTRS:
                #print('computed ', len(acc[feature]), 'windows for acc ', feature)
                DataHandler.STRESS_RESP_FEATURES[keys[key_index]].extend(acc[feature])
                key_index = key_index + 1
            
            eda = self.get_stats(data[index]['EDA'], window_size, window_shift)
            for feature in DataHandler.FEATURE_ATTRS:
                #print('computed ', len(eda[feature]), 'windows for eda ', feature)
                DataHandler.STRESS_RESP_FEATURES[keys[key_index]].extend(eda[feature])
                key_index = key_index + 1

            temp = self.get_stats(data[index]['Temp'], window_size, window_shift)
            for feature in DataHandler.FEATURE_ATTRS:
                #print('computed ', len(temp[feature]), 'windows for temp ', feature)
                DataHandler.STRESS_RESP_FEATURES[keys[key_index]].extend(temp[feature])
                key_index = key_index + 1
        return DataHandler.STRESS_RESP_FEATURES

    def get_train_and_test_data(self):
        X1 = []
        X2 = []
        for i in range(0, len(DataHandler.BASE_FEATURES['a_mean'])):
            X1.append([DataHandler.BASE_FEATURES['a_mean'][i], DataHandler.BASE_FEATURES['a_std'][i],\
                       DataHandler.BASE_FEATURES['a_maxx'][i], DataHandler.BASE_FEATURES['a_maxy'][i],\
                       DataHandler.BASE_FEATURES['a_maxz'][i], DataHandler.BASE_FEATURES['e_max'][i],\
                       DataHandler.BASE_FEATURES['e_min'][i],  DataHandler.BASE_FEATURES['e_mean'][i],\
                       DataHandler.BASE_FEATURES['e_range'][i],DataHandler.BASE_FEATURES['e_std'][i],\
                       DataHandler.BASE_FEATURES['t_max'][i],  DataHandler.BASE_FEATURES['t_min'][i],\
                       DataHandler.BASE_FEATURES['t_mean'][i], DataHandler.BASE_FEATURES['t_range'][i],\
                       DataHandler.BASE_FEATURES['t_std'][i]])
        #print(np.shape(X1))
        
        for i in range(0,  len(DataHandler.STRESS_RESP_FEATURES['a_mean'])):
            X2.append([DataHandler.STRESS_RESP_FEATURES['a_mean'][i], DataHandler.STRESS_RESP_FEATURES['a_std'][i],\
                       DataHandler.STRESS_RESP_FEATURES['a_maxx'][i], DataHandler.STRESS_RESP_FEATURES['a_maxy'][i],\
                       DataHandler.STRESS_RESP_FEATURES['a_maxz'][i], DataHandler.STRESS_RESP_FEATURES['e_max'][i],\
                       DataHandler.STRESS_RESP_FEATURES['e_min'][i], DataHandler.STRESS_RESP_FEATURES['e_mean'][i],\
                       DataHandler.STRESS_RESP_FEATURES['e_range'][i], DataHandler.STRESS_RESP_FEATURES['e_std'][i],\
                       DataHandler.STRESS_RESP_FEATURES['t_max'][i], DataHandler.STRESS_RESP_FEATURES['t_min'][i],\
                       DataHandler.STRESS_RESP_FEATURES['t_mean'][i], DataHandler.STRESS_RESP_FEATURES['t_range'][i],\
                       DataHandler.STRESS_RESP_FEATURES['t_std'][i]])                
        #print(np.shape(X2))
    ...
    def load_model(self, file_name=last_saved):
        ...
        file = (os.path.join(DataHandler.MODELS_DIR, file_name))
    ...
    def save_model(self, model):
        ...
        DataHandler.last_saved = str("model-" + \
                                     str(now.replace(microsecond=0)) +\
                                     ".h5").replace(" ", "").replace(":", "_")
        model.save(os.path.join(DataHandler.MODELS_DIR, DataHandler.last_saved))
        print("Saved model to disc:",\
              DataHandler.last_saved)
